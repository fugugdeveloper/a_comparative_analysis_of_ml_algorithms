...
def fetch_prs_with_retry(github_instance, repo_name, max_retries=6, retry_delay=5):
    cache_key = generate_cache_key(repo_name)
    cached_data = load_from_cache(cache_key)

    if cached_data:
        logging.info(f"Loaded data from cache for repository: {repo_name}")
        cache_repo = Repository.Repository(
            requester=github_instance._Github__requester,
            headers={},
            attributes=cached_data,
            completed=True
        )
        return cache_repo
    retries = 0
    while retries < max_retries:
        ...
        try:
            ...
            prs = github_instance.get_repo(repo_name).get_pulls(state='open')
            prs_data = [pr.raw_data for pr in prs]
            print("prs_data in fetch_prs_with_retry: ", prs_data)
            return github_instance.get_repo(repo_name).get_pulls(state='open')
...
def fetch_with_retry(github_instance, repo_name, max_retries=6, retry_delay=5):
    cache_key = generate_cache_key(repo_name)
    cached_data = load_from_cache(cache_key)
...
def process_file(repo, file_path):
    try:
...
def process_pull_request(github_instance, pr, repo_name):
    try:
        ...
        pr_details = {
            ...
            "review_time": sum([(review.submitted_at - pr.created_at).total_seconds() / 3600 for review in reviews if
                                review.submitted_at]),
        ...
        logging.error(f"Error processing PR: {e}")
        return None



def process_repo(github_instance, repo_name):
    cache_key = generate_cache_key(repo_name)
    cached_data = load_from_cache(cache_key)
    ...
    try:
        repo = fetch_with_retry(github_instance, repo_name)
        if repo:
            # print("fetched_repos: ", repo)
            prs = fetch_prs_with_retry(github_instance, repo_name)
        if prs:
            prs_data = [pr.raw_data for pr in prs]
            save_to_cache(prs_data, cache_key)
            for pr in prs: 
                pr_details = process_pull_request(github_instance, pr, repo_name)
                print("Pr_Details Data: ", pr_details)
        repo_data = []
        print("I'm here above concurrent ")

        if prs_data:
            print("pr_data in concurrent: ", prs_data)
            repo_data.append(prs_data)
            print("pr_data in concurrent: ", prs_data)
        save_to_cache(repo_data, cache_key)  # Save processed repository data to cache
        if repo_data:
            csv_path = f"{repo_name.replace('/', '_')}_dataset_prs.csv"
            df = pd.DataFrame(repo_data)
            if os.path.exists(csv_path):
                df.to_csv(csv_path, mode='a', header=False, index=False)
            else:
                df.to_csv(csv_path, index=False)
                print("saved data: ", repo_data)
        return repo_data

    except Exception as e:
        logging.error(f"Error processing repository: {repo_name}, {str(e.with_traceback)}")
        return None
...
def main():
    ...
    for repo_name in repo_names:
        ...
        repo_data = process_repo(github_instance, repo_name)

        if repo_data:
            output_path = os.path.join(output_dir, f"{repo_name.replace('/', '_')}_pr_data.json")
            with open(output_path, "w") as f:
                json.dump(repo_data, f, indent=4)
            print(f"Data for repository {repo_name} saved to {output_path}")
